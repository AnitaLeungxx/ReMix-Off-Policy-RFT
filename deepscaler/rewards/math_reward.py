...
import random
import time 
INITIAL_TIME = None  # 初始时间戳（首次调用时记录）
...
class RewardMathFn(RewardFn):
    ...
    def __call__(self, input: RewardInput) -> RewardOutput:
        ...
        is_correct_finalanswer = False
        strict_reward = self.config.incorrect_reward
        ...
        if THOUGHT_DELIMITER_START in model_response and THOUGHT_DELIMITER_END in model_response:
            ...
            strict_reward=self.config.format_error_reward
        ...
        if model_answer is None:
            strict_reward=self.config.format_error_reward
            is_correct_format = False
            is_correct_strict = is_correct_format and is_correct_finalanswer
            return RewardOutput(reward=strict_reward, is_correct=is_correct_strict, is_correct_format=is_correct_format, is_correct_finalanswer=is_correct_finalanswer)
            # return RewardOutput(reward=self.config.format_error_reward, is_correct=False)
        ...
        if not processed_ground_truths:
            if is_correct_format:
                strict_reward=self.config.unk_error_reward
                is_correct_format = False
                is_correct_strict = is_correct_format and is_correct_finalanswer
                return RewardOutput(reward=strict_reward, is_correct=is_correct_strict, is_correct_format=is_correct_format, is_correct_finalanswer=is_correct_finalanswer)
            # return RewardOutput(reward=self.config.unk_error_reward, is_correct=False)
        ...
        for ground_truth in processed_ground_truths:
            ...
            if is_correct:
                if is_correct_format:
                    strict_reward = self.config.correct_reward
                is_correct_finalanswer = True
...
def deepscaler_reward_fn(solution_str: str, ground_truth: Union[str, List[str]], enable_llm = False, do_print = None):
    ...
    if reward_response.is_correct:
        do_print = True
    if do_print:
       print(f"\n---------------Output & Reward Example-----------------\n⭐️Problem: {solution_str}\n⭐️Solution: {solution_str}\n----\n⭐️Ground_truth: {ground_truth}\n⭐️Reward: {reward_response.reward}. \t⭐️reward_response.is_correct: {reward_response.is_correct}\n⭐️Format correct: {reward_response.is_correct_format}\t⭐️Final Answer correct: {reward_response.is_correct_finalanswer}\n--------------- - - - - - - - - - - - ----------------\n")
    return reward_response.is_correct_finalanswer

def deepscaler_reward_fn_for_evaluator(solution_str: str, ground_truth: Union[str, List[str]], enable_llm = False, do_print = None):
    reward_config = RewardConfig()
    reward_config.use_math_orm = enable_llm
    reward_fn = RewardMathFn(reward_config)
    reward_response = reward_fn(RewardInput(problem=solution_str, problem_type=RewardType.MATH, model_response=solution_str, ground_truth={"answer": ground_truth}))
    ...
    if do_print is None:
        do_print = random.randint(1, 128)
    if reward_response.is_correct == True:
        do_print = 1
        
    if do_print:
        print(f"\n---------------Output & Reward Example-----------------\n⭐️Solution: {solution_str}\n--------------\n⭐️Ground_truth: {ground_truth}\n⭐️Reward: {reward_response.reward}. \t⭐️reward_response.is_correct: {reward_response.is_correct}\n⭐️Format correct: {reward_response.is_correct_format}\t⭐️Final Answer correct: {reward_response.is_correct_finalanswer}\n--------------- - - - - - - - - - - - ----------------\n")
    return reward_response.is_correct, reward_response.reward, reward_response.is_correct_format, reward_response.is_correct_finalanswer
